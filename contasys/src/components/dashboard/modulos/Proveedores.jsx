...
export default function Proveedores() {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      setFormErrors({ _form: e?.message || 'Error guardando proveedor' })
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleDelete(row) {
    ...
    setLoading(true)
    try {
      const { error: err } = await supabase.from('proveedores').delete().eq('id', row.id)
      if (err) throw err
      setPage(1)
    } catch (e) {
  ...
  if (loadingEmpresa) {
    return <div style={{ marginLeft: 240, padding: 24 }}>Cargando proveedores...</div>
  }
  ...
  return (
    <div style={{ marginLeft: 240, padding: 24, paddingTop: 92 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 1000, color: '#04342C' }}>Proveedores</h2>
          <p style={{ margin: '6px 0 0', opacity: 0.7 }}>Gestiona tu catálogo de proveedores.</p>
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 1000, color: '#04342C' }}>
                  {modalMode === 'add' ? '+ Agregar proveedor' : 'Editar proveedor'}
                </h3>
                <p style={{ margin: '6px 0 0', opacity: 0.7, fontWeight: 800 }}>Completa los datos del proveedor.</p>
              </div>
              <button
                type="button"
